...
class DataHandler:
    ...
    def create_data(self, size = 48000):
        ...
        os.makedirs("data\\test_data", exist_ok = True)
        ...
        for file in os.listdir('data\\audio'):
            with open(file, 'r', encoding = "utf-8") as current_file:
                ...
                for j in self.indices_current_file:
                    ...
                    if j % 2 == 0:
                        wavfile.write(f"data\\training_data\\{j}_{current_file.name}_training", self.samplerate, current_file[begin:end])
                    else:
                        wavfile.write(f"data\\test_data\\{j}_{current_file.name}_test", self.samplerate, current_file[begin:end])
